...
- 变轨策略与 delta-v / 转移时间复核，支持从 `config/maneuver_strategy.json` 和 `data/maneuver_snapshot.json` 读取参数。
- 发射窗口约束分析，支持采样步长、测控资源、太阳角、地影、倾角等参数解释与复核。
...
- 不在模型响应中声称已重新积分或重新采样，除非工具或上下文明确提供新计算结果。
