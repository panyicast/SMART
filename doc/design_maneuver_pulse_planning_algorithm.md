...
结果表 `控后近地点高度/km` 列中，MV1 和 MV2 的前段远地点变轨单元格为高亮可编辑。用户修改该单元格后，页面会把数值写入 `hard_constraint_planner.fixed_hp_targets_km` 的 `1` 或 `2`，随后自动保存并重新计算优化策略。
...
算法在 `earth.use_J2 = True` 时使用笛卡尔状态数值积分传播 A/P 点事件，动力学为地球二体引力加 J2 加速度。导出的轨道根数为由瞬时位置速度反算的瞬时根数。关闭 `earth.use_J2` 时才回退为二体解析传播。
...
寻找下一 A/P 点时使用径向速度 `dot(r, v) / |r|` 过零事件检测：由正到负为远地点，由负到正为近地点。事件点、点火前状态和导出表中的根数均来自该瞬时状态。
