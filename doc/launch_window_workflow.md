...
- 主页面左侧仅显示轨道来源、发射开始/结束时刻、火箭飞行时间、扫描步长、状态设置概览和计算操作。
- 点击“状态设置”打开独立对话框，集中编辑地面站、中继星、可见性阈值、点火期间太阳角/帆板方向和限制条件表。
- 状态设置对话框点击“取消”会放弃本次编辑；点击“应用状态设置”只更新页面内存配置，仍需点击主页面“保存参数”或“计算发射窗口”才写入项目配置。
...
页面同时写入 `data/launch_window_samples.meta.json`，记录发射窗口配置、变轨策略和轨道历史文件 hash。项目加载时只在 metadata 与当前输入一致时读取样本缓存并重建结果表和甘特图；修改约束、阈值、扫描步长、测控资源、变轨策略或轨道历史后，需要重新点击“计算发射窗口”刷新缓存。
