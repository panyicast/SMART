...
- Current continuous-thrust parameter update: added a lower-left "连续推力模型参数" card with an "优化连续推力模型参数" button and compact MV table. The first-pass service uses the current pulse plan to produce sequential per-maneuver parameters: burn start time, fixed yaw angle, cutoff time, target post-burn semi-major axis, and objective `ΔG`. It records the reference search contract (`t` step 10 s, yaw step 0.05 deg) and objective terms (`m + m1 + m2 + m3` for non-final burns, `m + m3` for final burn). Risk/follow-up: this is a pulse-derived parameter optimization scaffold; the next minimum task is replacing it with full finite-thrust propagation/grid enumeration where cutoff is triggered by reaching the target semi-major axis.
